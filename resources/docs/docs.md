...
The `home-routes` handler in the `rhizome.routes.home` namespace
...
The `home-page` function will in turn call the `rhizome.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/rhizome/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `rhizome.handler` namespace:
...
Request middleware functions are located under the `rhizome.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rhizome.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `rhizome.db.core` in some other namespace.
